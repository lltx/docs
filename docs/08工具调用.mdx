...
## 低级工具API
使用 `ChatLanguageModel` 的 `generate(List<ChatMessage>, List<ToolSpecification>)` 方法，可以在低级别上管理函数调用。
...
+ **自动函数调用**：当LLM决定调用工具时，AI服务会自动执行相应方法，返回值将直接发送给LLM。对于复杂场景，可以使用 `@P` 注解描述参数、使用 `@ToolMemoryId` 处理多用户、多会话等场景。
